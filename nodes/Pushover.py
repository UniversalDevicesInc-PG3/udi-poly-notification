...
class Pushover(Node):
    ...
    def config_info_nr(self):
        ...
        info = [
            '<h4>Example Network Resource settings for Pushover</h4><ul><li>http<li>POST<li>Host:{0}<li>Port:{1}<li>Path: /send?node={2}&Subject=My+Subject&monospace=1&device=1&priority=2<li>Encode URL: not checked<li>Timeout: 5000<li>Mode: Raw Text</ul>'.format(rest_ip,rest_port,self.address),
    ...
    def cmd_send_sys_short_with_params(self,command):
        ...
        if msg['body'] == '' or msg['body'] == ' ':
            # Pushover must have a message, title is optional
