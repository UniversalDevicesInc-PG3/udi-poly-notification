...
The Service nodes allow you to send messages to a service, currently the only supported service is Pushover.  You must also define at least one Service Node. Name, which will be used in drop down menus when programming. See below. 
...
A Notify node accepts a Device ON / Device OFF from a scene or a program. Create a Notify node in the Configuration using "Add Notify Nodes” as follows:
  ...
  - Service Node Name: Set to match to the Name of an existing Service Node you created abiove. 
- Press 'Save Changes'
- Press 'Restart'
